...
# Your code here
files_list <- list.files("C:/Users/fabisc/OneDrive - Karolinska Institutet/Documents/Courses/Intermediate R/Day6_Quiz/Timeseries_data/", pattern = "csv", full.names = TRUE, recursive = T)
files_list
?list.files()
...
# Your code here
data_list <- purrr::map(files_list, ~ read.csv(.)) #why (.)?
data_list
?purrr::map
...
combined_df %>%
  group_by(Category) %>% 
  summarise(n_missing = sum(is.na(Value))) #what is n_missing?
?summarise
```
...
mdn_df <- wide_df %>%
  group_by(Date) %>%  #works without ungroup
  mutate(Mdn_value = median(c_across(A:C), na.rm = T))
...
last_3_rows <- recode_df %>%
  slice_tail(n = 3) #3 of each category
last_3_rows
...
# tages top10 row df and makes it wider gain 
processed_df <- top_10_rows %>% 
  pivot_wider(names_from = 'Category', values_from = 'Value') 
  
# takes wider df, binds columns, that are result of applying the function in function list 
# the rename the new columns and put it into the dataframe
...
# Your code here
ggplot(mdn_df, aes(x = Month, y = Mdn_value, color = Month)) +
  geom_line() +
...
# Your code here
processed_df %>% 
  separate(cols = -c(Date, Month), names_to = 'Name', values_to = 'Value') %>% 
